...
$(document).ready(function () {
    $("#update").on("click",function(e){
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('Opportunity.update', $opportunity->id) }}",
            data: formData,
            dataType: 'json',
            cache: false,
            contentType: false,
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">@lang('words.opportunity-edit') ( {{ $opportunity->title }} )</h4>
                            </div>
                            <form id="opportunity_form">
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="name">@lang('words.opportunity-title')</label>
                                        <input type="text" class="form-control" id="title" value="{{ $opportunity->title }}">
                                    </div>
                                    <div class="form-group">
                                            <label>@lang('words.opportunity-product')</label>
                                            <select class="select2 form-control" id="product_id">
                                                @foreach ($_products as $product)
                                                <option @if($product->id == $opportunity->product_id) selected @endif value="{{ $product->id }}">{{ $product->title }}</option>
                                                @endforeach
                                            </select>
                                    </div>
                                    <div class="form-group">
                                            <label for="fp-date-time">@lang('words.opportunity-end-date-label')</label>
                                            <input type="text" id="end" class="form-control flatpickr-date-time" value="{{ $opportunity->end }}" placeholder="YYYY-AA-GG SS:DD" />
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group" id="updated_image">
                                        ...
                                        <img width="120" src="{{ asset($opportunity->image) }}">
                                        </div>
                                    </div>
